...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await db
      .update(leads)
      .set({
        showedCallAt: new Date(),
        updatedAt: new Date(),
      })
      .where(eq(leads.id, lead.id));

    // Fire CAPI ShowedCall event
    const eventId = `sc.${Date.now()}.${Math.random().toString(36).slice(2, 10)}`;
    await sendCAPIEvent({
      event_name: "ShowedCall",
      ...
      custom_data: {
        content_name: "Showed Call",
      },
    }).catch(() => {});

    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error("[Meta Webhook] ShowedCall error:", error);
    return NextResponse.json(
