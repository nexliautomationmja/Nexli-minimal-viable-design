...
// ─────────────────────────────────────────────────────────────────────────────
// REVENUE LEAK CALCULATOR — Interactive Lead Magnet
// ─────────────────────────────────────────────────────────────────────────────
const RevenueLeakCalculator: React.FC = () => {
  const { openBooking } = useBooking();
  ...
  const calculateOpportunity = () => {
    ...
    // Firms $250K-500K: ~$150/hr, $500K-1M: ~$175/hr, $1M+: ~$200/hr
    let hourlyRate = 150;
    if (revenue > 1000000) hourlyRate = 200;
    else if (revenue > 500000) hourlyRate = 175;

    // Time waste cost (admin hours × implied hourly rate × 52 weeks)
    const timeWasteCost = hours * hourlyRate * 52;

    // INDUSTRY BENCHMARK: Lost opportunity cost
    // CPA firms typically receive 8-15 inquiries/month based on size
    // Slow response = 27% conversion loss vs. fast response (industry data)
    // Average client value = revenue / 50 clients (industry avg for firms this size)
    const estimatedInquiries = revenue < 500000 ? 8 : revenue < 1000000 ? 12 : 15;
    const avgClientValue = Math.max(revenue / 50, 3000); // Min $3K per client
    const conversionLoss = 0.27; // 27% average loss from slow follow-up
    const lostOpportunityCost = estimatedInquiries * conversionLoss * avgClientValue * 12;

    // Review gap impact (competitors average 50+ reviews, each missing review = ~$2,400/year in lost trust)
    ...
    try {
      ...
      await fetch('https://services.leadconnectorhq.com/hooks/aFlQmmyaRZncBaqSQz2L/webhook-trigger/d22e5d91-61de-46ac-ad9a-8e7a6e7a0fa1', {
        ...
        body: JSON.stringify({
          ...
          lastName,
          email,
          source: 'vslfunnel-growth-calculator',
  ...
  return (
    <section className="relative py-14 sm:py-20 md:py-28 px-4" style={{ backgroundColor: '#0a1018' }}>
      {/* Background effects */}
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        >
          ...
          <h2
            className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black leading-tight mb-3 sm:mb-4"
            style={{ fontFamily: "'Syne', sans-serif", color: '#ffffff' }}
          >
            Calculate Your Firm&apos;s{' '}
            <span className="bg-gradient-to-r from-red-400 to-orange-500 bg-clip-text text-transparent">
              Revenue Leak
            </span>
          </h2>
          <p className="text-sm sm:text-base md:text-lg leading-relaxed max-w-2xl mx-auto" style={{ color: 'rgba(255,255,255,0.65)' }}>
            See exactly how much money you&apos;re leaving on the table with outdated systems. Takes 60 seconds.
          </p>
        </motion.div>
          ...
          >
            ...
            {!emailCaptured ? (
              ...
              >
                ...
                <p className="text-sm sm:text-base mb-6" style={{ color: 'rgba(255,255,255,0.65)' }}>
                  Your full diagnostic report is on its way. Now let&apos;s talk about plugging these leaks.
                </p>
              </motion.div>
            )}
            ...
            >
              ...
              <p className="mt-3 text-xs sm:text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>
                Book a strategy call to see exactly how to plug these leaks
              </p>
            </motion.div>
